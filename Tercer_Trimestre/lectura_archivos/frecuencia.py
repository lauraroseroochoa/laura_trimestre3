...
counters = {chr(ch): 0 for ch in range(ord('a'), ord('z') + 1)} # se llena por compresion el diccionario con cada letra del abecedario 
file_name = input("Ingresa el nombre del archivo a analizar: ")
try:
    file = open(file_name, "rt") # se abre el archivo
    for line in file:
        for char in line:
            # Si es una letra...
            if char.isalpha():
